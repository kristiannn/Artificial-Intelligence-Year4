...
turtles-own [
  ...
  desired-speed ; speed driver wants to drive
  target-lane   ; the desired lane of the car
  patience      ; the driver's current level of patience
  speed-difference
  experience    ; how experienced the driver behind the wheel is
  mistake-chance
  perfect-speed ; the perfect is the speed the driver should be driving at (inexperienced drivers may need to drive slower than they want to)
  crashes
  heavy-crashes
  mistakes
  no-mistake
...
to create-or-remove-cars
  ...
  create-turtles (number-of-cars - count turtles) [
    ...
    set target-lane last lanes ; 1 is left lane -1 is right lane , pycor is default value
    set heading 90
    set top-speed 0.6
    set speed 0.2
    set patience random max-patience
    set desired-speed 0.4 + random-float 0.2 ; set to a random value, 0.7 is for testing purposes!
    set speed-difference 0.0
    set experience 0.95 + random-float 0.05 ; experience of each driver is a random value of 0.9 to 1.0
    set mistake-chance 0 ; debugging purposes
    set perfect-speed desired-speed * experience
    set mistakes 0
    set no-mistake 0
  ]
...
to draw-road
  ask patches [
    ...
    set pcolor green - random-float 0.2
  ]
...
to go
  create-or-remove-cars
  ask turtles [ calculate-mistake-chance ]
  ask turtles [ move-forward ]
  ;ask turtles with [ patience <= 0 ] [ choose-new-lane ]
  ask turtles with [ ycor != target-lane ] [ move-to-target-lane ]
  ask turtles [ collision-check ]
  tick
end

to calculate-mistake-chance
  set mistake-chance 0 ; debugging purposes, just in case
  let speed-to-perf 0

  ;if above perfect speed - increase mistake chance!
  if speed > perfect-speed
  [
    set speed-to-perf abs (speed - perfect-speed) ; absolute value for debugging purposes, shouldn't make a difference ever
  ]

  let speed-to-desired abs (speed - desired-speed) / 25  ; speed != desired speed isn't as bad
  set mistake-chance ((speed-to-perf + speed-to-desired) / experience) / 30
end
...
to move-forward ; turtle procedure
  ...
  let cars-ahead other turtles in-cone 10 180 with [ y-distance <= 1 ] ; 1 + speed is distance before we start braking, 180 is cone angle
  let car-ahead min-one-of cars-ahead [ distance myself ]
  ifelse car-ahead != nobody [set speed-difference abs (speed - [speed] of car-ahead)] [set speed-difference speed]

  let blocking-cars other turtles in-cone (2 + speed-difference * (6 * experience)) 180 with [ y-distance <= 1 ] ; 1 + speed is distance before we start braking, 180 is cone angle
  ...
  [
    ...
    [
      let me self
      let dist [distance blocking-car] of self
      if dist < (1 * experience)
      [
        slow-down-car
      ]
      ifelse speed > [speed] of blocking-car
      [
    ...
    [
      ...
      let mistake random-float 1 ; generate random number to decide if AI makes mistake or not
      ifelse mistake > mistake-chance
      [
        set no-mistake no-mistake + 1
        let car-other-lane other turtles in-cone (8 - speed * 4) 180 with [ ycor = 1] ; removing the speed because if you drive fast you wouldn't care to look too far behind
        let cars-other-lane min-one-of car-other-lane [distance myself]
        if cars-other-lane = nobody
        [
          if speed > [speed] of blocking-car
          [
            ; if speed difference is too big, slow down a bit to not crash into car ahead
            ifelse speed * 1.8 > [speed] of blocking-car
            [
              ;set speed [speed] of blocking-car * 1.8
              slow-down-car
              set target-lane first lanes
              set color pink;
            ]
            [
              set target-lane first lanes
        ...
        [
          let me self
          let dist [distance blocking-car] of self
          if dist < (1 * experience) ; drivers with less experience keep more dangerous distances (lower) to car ahead
          [
            slow-down-car
          ]
          ifelse speed > [speed] of blocking-car
      ...
      [
        set target-lane first lanes
        set color white;
        set mistakes mistakes + 1 ; count mistakes
      ]
  ...
  [
    ...
    [
      set heading 110
      let car-right-lane other turtles in-cone (5 + ((speed * 4) * experience)) 180 with [ ycor = -1 ]
      let cars-right-lane min-one-of car-right-lane [distance myself]
      if cars-right-lane = nobody
      [
        set target-lane last lanes
        set color green;
      ]
      set heading 90
    ]
...
to collision-check
  ; only check for collisions while in-lane driving since lane-change is a bit messy
  ; on improper lane-change, cars would create the problem once they have completely
  ; swapped lanes, so it won't alter results either way
  if ycor = 1 or ycor = -1
  [
    let crash-cars other turtles in-cone 0.65 45 with [ ycor = -1 or ycor = 1 ] ; ycors are because only in-lane collisions should be counted
    let crash-car min-one-of crash-cars [ distance myself ]
    if crash-car != nobody
    [
      let me self
      let spdd abs (([speed] of self) - ([speed] of crash-car))
      ifelse spdd > 0.2
      [
        set heavy-crashes heavy-crashes + 1
      ]
      [
        set crashes crashes + 1
      ]
      set color black
      ; slow down massively - debugging purposes to not count a crash many times
      set speed ([speed] of crash-car) - 0.1
...
to slow-down-car ; turtle procedure
  set speed (speed - (deceleration + speed-difference / 10))
...
to speed-up-car ; turtle procedure
  if speed < desired-speed [ set speed (speed + acceleration) ]
  if speed > desired-speed [ set speed (speed - deceleration) ] ; shouldn't matter, just debugging purposes
...
620
Mistakes-chance
mean [mistake-chance] of turtles
17
1
11
